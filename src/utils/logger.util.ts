...
class Logger {
	...
	_log(
		...
		try {
			...
			// If we can't write to the log file, only log to console if not in STDIO mode
			if (process.env.TRANSPORT_MODE !== 'stdio' && process.env.NODE_ENV === 'development') {
				console.error(`Failed to write to log file: ${err}`);
			}
		}

		// Only output to console in test mode or when not using STDIO transport
		// STDIO mode requires clean stdout/stderr for JSON-RPC communication
		if (process.env.NODE_ENV === 'test') {
			console[level](logMessage);
		} else if (process.env.TRANSPORT_MODE !== 'stdio' && process.env.NODE_ENV === 'development') {
			console.error(logMessage);
		}
		// In STDIO mode, only write to log file, no console output
	}
