...
import campaignResources from './resources/campaigns.resource.js';
...
export async function startServer(
	...
	campaignTools.registerTools(serverInstance);
	campaignResources.registerResources(serverInstance);
 ...
 */
async function main() {
	// Suppress dotenv console output for STDIO mode BEFORE any config loading
	const originalConsoleLog = console.log;
	const originalConsoleWarn = console.warn;
	
	if (process.env.TRANSPORT_MODE === 'stdio' || process.argv.length === 2) {
		console.log = () => {};
		console.warn = () => {};
	}
	...
	// Restore console methods after config loading
